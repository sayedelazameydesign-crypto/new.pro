// ===== طبقة قاعدة بيانات الحسابات (Neon) — جدول المستخدمين يُنشأ تلقائيًا =====
...
export async function findUserByEmail(email: string): Promise<UserRow | null> {
  await ensureTables();
  const rows = (await db()`SELECT id, email, name, password_hash FROM nahwa_users WHERE email = ${email} LIMIT 1`) as UserRow[];
  return rows?.[0] ?? null;
}
